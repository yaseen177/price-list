...
const TableCell = ({ isSelected, onClick, children, colSpan = 1 }: { isSelected: boolean, onClick: () => void, children: React.ReactNode, colSpan?: number }) => (
  <td
    colSpan={colSpan}
    onClick={onClick}
    className={`p-2 md:p-3 border border-gray-300 text-center cursor-pointer transition-colors text-xs sm:text-sm md:text-base ${
...
const VarifocalDemo = ({ readingAdd }: { readingAdd: number }) => {
  ...
  return (
    <div className="mt-4 md:mt-6 p-4 md:p-6 bg-slate-800 rounded-xl text-white shadow-inner">
      <h3 className="text-base md:text-lg font-bold mb-2">Live Varifocal Visualiser</h3>
      <div className="flex flex-wrap gap-2 mb-6 justify-center md:justify-start">
        {scenarios.map(s => (
          <button key={s.id} onClick={() => setActiveScenario(s)} className={`px-3 md:px-4 py-2 text-xs md:text-sm font-semibold rounded-lg transition-all flex-1 md:flex-none whitespace-nowrap ${activeScenario.id === s.id ? 'bg-[#3f9185] text-white' : 'bg-slate-700 text-slate-300'}`}>{s.name}</button>
        ))}
      </div>
        ...
        {(['Basic', 'Elite', 'Individual'] as const).map((tier) => {
          ...
          return (
            <div key={tier} className="flex flex-col items-center">
              <h4 className="font-semibold text-[#3f9185] bg-white px-3 py-1 rounded-full text-xs md:text-sm mb-3 shadow">{tier}</h4>
              <div className="relative w-full h-48 sm:h-64 bg-slate-200 rounded-t-[40%] rounded-b-[40%] overflow-hidden border-4 border-slate-600 shadow-lg" style={{ backgroundImage: `url('${activeScenario.img}')`, backgroundSize: 'cover', backgroundPosition: activeScenario.bgPos }}>
    ...
    <div className="flex flex-col items-center justify-end w-full h-[350px] sm:h-[400px] lg:h-[480px] relative bg-white border-2 border-gray-100 rounded-3xl shadow-sm pb-16 sm:pb-24 mt-4">
      ...
      <div className="absolute top-4 sm:top-6 left-0 right-0 flex flex-col items-center px-2 sm:px-4 text-center z-20">
        ...
        <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
          {lighterLens !== 'EQUAL' && weightA !== 0 && (
            <span className="px-3 sm:px-5 py-1.5 sm:py-2 bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-full text-[10px] sm:text-sm font-black uppercase tracking-widest shadow-sm">
              Lens {lighterLens} is {weightDiffText}
            </span>
          )}
          {thinnerLens !== 'EQUAL' && weightA !== 0 && (
            <span className="px-3 sm:px-5 py-1.5 sm:py-2 bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-full text-[10px] sm:text-sm font-black uppercase tracking-widest shadow-sm">
              Lens {thinnerLens}'s {thickDiffText}
            </span>
          )}
        </div>
      </div>
      ...
      <div className="w-[200px] sm:w-[280px] md:w-[480px] h-2 sm:h-3 bg-slate-800 rounded-full relative transition-transform duration-1000 ease-out z-10" style={{ transform: `rotate(${angle}deg)` }}>
        ...
        <div className="absolute left-0 bottom-full w-24 sm:w-32 md:w-36 -translate-x-1/2 transition-transform duration-1000 ease-out origin-bottom flex flex-col items-center" style={{ transform: `rotate(${-angle}deg)` }}>
          ...
          <div className="w-full h-1.5 sm:h-2.5 bg-slate-700 rounded-full shadow-lg z-0"></div>
          <div className="w-2 sm:w-3 h-2 sm:h-3 bg-slate-500 rounded-b-sm z-0"></div>
        </div>
        
        <div className="absolute left-0 top-full mt-2 sm:mt-3 w-24 sm:w-32 md:w-36 -translate-x-1/2 transition-transform duration-1000 ease-out origin-top flex flex-col items-center" style={{ transform: `rotate(${-angle}deg)` }}>
          <div className="flex flex-col items-center justify-center bg-white/90 backdrop-blur px-2 sm:px-4 py-2 sm:py-3 rounded-xl border sm:border-2 border-gray-200 shadow-md w-full">
            <span className="text-[8px] sm:text-[10px] font-black uppercase text-gray-400 tracking-widest">Lens A</span>
            <div className={`font-black text-lg sm:text-2xl md:text-3xl leading-none mt-0.5 sm:mt-1 ${lighterLens === 'A' ? 'text-emerald-500' : lighterLens === 'B' ? 'text-red-500' : 'text-slate-700'}`}>{weightA}g</div>
            <div className={`text-[10px] sm:text-xs font-bold mt-0.5 sm:mt-1 ${thinnerLens === 'A' ? 'text-emerald-500' : thinnerLens === 'B' ? 'text-red-500' : 'text-slate-500'}`}>{thickA}mm</div>
          </div>
        </div>
        ...
        <div className="absolute right-0 top-full mt-2 sm:mt-3 w-24 sm:w-32 md:w-36 translate-x-1/2 transition-transform duration-1000 ease-out origin-top flex flex-col items-center" style={{ transform: `rotate(${-angle}deg)` }}>
          <div className="flex flex-col items-center justify-center bg-white/90 backdrop-blur px-2 sm:px-4 py-2 sm:py-3 rounded-xl border sm:border-2 border-gray-200 shadow-md w-full">
            <span className="text-[8px] sm:text-[10px] font-black uppercase text-gray-400 tracking-widest">Lens B</span>
            <div className={`font-black text-lg sm:text-2xl md:text-3xl leading-none mt-0.5 sm:mt-1 ${lighterLens === 'B' ? 'text-emerald-500' : lighterLens === 'A' ? 'text-red-500' : 'text-slate-700'}`}>{weightB}g</div>
            <div className={`text-[10px] sm:text-xs font-bold mt-0.5 sm:mt-1 ${thinnerLens === 'B' ? 'text-emerald-500' : thinnerLens === 'A' ? 'text-red-500' : 'text-slate-500'}`}>{thickB}mm</div>
          </div>
        </div>
...
const LensThicknessVisualiser = () => {
  ...
  return (
    <>
      <div className="bg-slate-900 rounded-2xl p-6 md:p-8 text-white shadow-xl mb-6 md:mb-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h3 className="text-lg md:text-xl font-black uppercase tracking-tight text-white">Lens Thickness & Weight Visualiser</h3>
            <p className="text-slate-400 text-xs md:text-sm font-medium mt-1">Compare physical profiles and weight across different refractive indices.</p>
          </div>
          <button 
            onClick={() => setIsOpen(true)} 
            className="w-full md:w-auto px-6 md:px-10 py-3 bg-[#3f9185] text-white font-black rounded-xl shadow-lg hover:scale-105 transition-all uppercase tracking-widest text-xs whitespace-nowrap"
          >
            Launch Visualiser
          </button>
        </div>
          ...
          <div className="bg-gray-50 rounded-2xl shadow-2xl w-full max-w-6xl max-h-[98vh] md:max-h-[95vh] flex flex-col relative border-2 md:border-4 border-[#3f9185] overflow-hidden">
            ...
            <div className="shrink-0 bg-white border-b border-gray-200 p-4 md:p-6 flex flex-col md:flex-row justify-between items-start md:items-center z-20 gap-3 md:gap-4">
              <div className="w-full md:w-auto flex justify-between items-center">
                <div>
                  <h2 className="text-xl md:text-2xl font-black text-slate-800 uppercase tracking-tight">Lens Weigh-In</h2>
                </div>
              ...
              <div className="flex bg-gray-100 p-1.5 rounded-xl border border-gray-200 w-full md:w-auto overflow-x-auto">
                <button 
                  ...
                  className={`flex-1 md:flex-none px-4 sm:px-6 py-2 md:py-2.5 text-[10px] md:text-xs font-black rounded-lg transition-all uppercase tracking-widest whitespace-nowrap ${activeTab === 'compare' ? 'bg-white text-slate-800 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                >
                  Head-to-Head
                </button>
                <button 
                  onClick={() => setActiveTab('grid')}
                  className={`flex-1 md:flex-none px-4 sm:px-6 py-2 md:py-2.5 text-[10px] md:text-xs font-black rounded-lg transition-all uppercase tracking-widest whitespace-nowrap ${activeTab === 'grid' ? 'bg-white text-slate-800 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                >
                  All Indices
                </button>
              </div>
              ...
              {activeTab === 'compare' ? (
                  ...
                  <div className="w-full lg:w-[35%] flex flex-col gap-4 sm:gap-6 shrink-0">
                    ...
                    <div className="bg-white p-4 md:p-6 rounded-2xl border-2 border-[#3f9185]/30 shadow-md flex flex-col">
                      <div className="flex justify-between items-center mb-4 md:mb-6">
                        ...
                        <span className={`text-lg md:text-2xl font-mono font-black px-3 md:px-4 py-1 md:py-1.5 rounded-lg border-2 shadow-inner ${prescription > 0 ? 'text-blue-600 border-blue-200 bg-blue-50' : prescription < 0 ? 'text-red-500 border-red-200 bg-red-50' : 'text-gray-500 border-gray-200 bg-white'}`}>
                      ...
                      <input 
                        ...
                        className="w-full h-2 sm:h-3 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-[#3f9185]" 
                      />
                      <div className="flex justify-between text-[8px] md:text-[10px] font-black text-gray-400 mt-2 md:mt-3 uppercase tracking-widest">
                        <span>Minus (-10)</span>
                        <span>Plano</span>
                        <span>Plus (+8)</span>
                      </div>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-1 gap-3 md:gap-4">
                      <div className="bg-white p-3 md:p-5 rounded-2xl border border-gray-200 shadow-sm text-center">
                        <label className="text-[8px] md:text-[10px] font-black uppercase text-gray-400 tracking-widest block mb-2 md:mb-3">Select Lens A</label>
                        <select 
                      ...
                      <div className="bg-white p-3 md:p-5 rounded-2xl border border-gray-200 shadow-sm text-center">
                        <label className="text-[8px] md:text-[10px] font-black uppercase text-gray-400 tracking-widest block mb-2 md:mb-3">Select Lens B</label>
                        <select 
                ...
                <div className="flex flex-col">
                  <div className="mb-6 md:mb-10 bg-white p-4 md:p-6 rounded-2xl border border-gray-200 shadow-sm">
                    <div className="flex justify-between items-center mb-4 md:mb-6">
                      <span className="text-xs md:text-sm font-black text-gray-500 uppercase tracking-widest">Prescription Slider</span>
                      <span className={`text-xl md:text-3xl font-mono font-black px-4 md:px-6 py-1 md:py-2 rounded-xl border-2 shadow-sm ${prescription > 0 ? 'text-blue-600 border-blue-200 bg-blue-50' : prescription < 0 ? 'text-red-500 border-red-200 bg-red-50' : 'text-gray-500 border-gray-200 bg-white'}`}>
                        {prescription > 0 ? '+' : ''}{prescription.toFixed(2)}
                      </span>
                    </div>
                    <input 
                      type="range" min="-10" max="8" step="0.25" value={prescription} 
                      onChange={(e) => setPrescription(parseFloat(e.target.value))} 
                      className="w-full h-3 md:h-4 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-[#3f9185]" 
                    />
                    <div className="flex justify-between text-[10px] md:text-xs font-black text-gray-400 mt-3 md:mt-4 uppercase tracking-widest">
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-6 md:py-10 px-2 sm:px-4 font-sans text-slate-900">
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-200">
        <div className="bg-white py-4 md:py-6 px-4 md:px-8 flex justify-center border-b-4 border-[#3f9185]">
          <img src="https://img1.wsimg.com/isteam/ip/297dd456-b70f-4175-b7e6-7f1aabf6e6b3/blob-0f9c396.png/:/rs=w:315,h:63,cg:true,m/cr=w:315,h:63/qt=q:95" alt="Logo" className="h-10 md:h-14 object-contain" />
        </div>

        <div className="p-4 sm:p-6 md:p-8 space-y-6 md:space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
            <div>
              <label className="block text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-1 md:mb-2">Patient Name</label>
              <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} placeholder="Enter name..." className="w-full p-2 md:p-3 border-2 border-gray-100 rounded-xl focus:border-[#3f9185] bg-gray-50 outline-none font-bold transition-all text-sm md:text-base" />
            </div>
            <div>
              <label className="block text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-widest mb-1 md:mb-2">Frame Choice</label>
              <div className="flex bg-gray-100 p-1 rounded-xl border-2 border-gray-100">
                <button onClick={() => setIsOwnFrame(false)} className={`flex-1 py-2 text-[10px] md:text-xs font-black rounded-lg transition-all uppercase tracking-widest ${!isOwnFrame ? 'bg-[#3f9185] text-white shadow-lg' : 'text-gray-400'}`}>New Frame</button>
                <button onClick={() => setIsOwnFrame(true)} className={`flex-1 py-2 text-[10px] md:text-xs font-black rounded-lg transition-all uppercase tracking-widest ${isOwnFrame ? 'bg-[#3f9185] text-white shadow-lg' : 'text-gray-400'}`}>Own Frame</button>
              </div>
            </div>
            <div className="sm:col-span-2 md:col-span-1">
              <label className={`block text-[10px] md:text-xs font-black uppercase tracking-widest mb-1 md:mb-2 ${isOwnFrame ? 'text-gray-200' : 'text-gray-400'}`}>Frame Price (£)</label>
              <input type="number" disabled={isOwnFrame} value={framePrice || ''} onChange={(e) => setFramePrice(parseFloat(e.target.value))} placeholder="0.00" className="w-full p-2 md:p-3 border-2 border-gray-100 rounded-xl bg-gray-50 outline-none font-black text-base md:text-lg transition-all" />
            </div>
          </div>
          ...
          {lensType.includes('Varifocal') && (
            <div className="bg-slate-900 rounded-2xl p-6 md:p-8 text-white shadow-xl">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4 md:mb-6">
                <div>
                  <h3 className="text-lg md:text-xl font-black uppercase tracking-tight">Varifocal Corridor Visualiser</h3>
                  <p className="text-slate-400 text-xs md:text-sm font-medium mt-1">Demonstrate vision zones based on Reading Addition.</p>
                </div>
                <button onClick={() => setShowDemo(!showDemo)} className="w-full md:w-auto px-6 md:px-10 py-3 bg-[#3f9185] text-white font-black rounded-xl shadow-lg hover:scale-105 transition-all uppercase tracking-widest text-xs whitespace-nowrap">{showDemo ? 'Close Demo' : 'Launch Demo'}</button>
              </div>
              {showDemo && (
                <div className="mt-6 md:mt-8 pt-6 md:pt-8 border-t border-slate-800">
                  <label className="block text-[10px] md:text-xs font-black uppercase tracking-widest text-slate-500 mb-3 md:mb-4">Reading Addition: <span className="text-[#3f9185] text-lg md:text-xl ml-2">+{readingAdd.toFixed(2)}</span></label>
                  <input type="range" min="1.00" max="3.50" step="0.25" value={readingAdd} onChange={(e) => setReadingAdd(parseFloat(e.target.value))} className="w-full h-2 bg-slate-800 rounded-lg appearance-none accent-[#3f9185] mb-6 md:mb-8" />
          ...
          <div className="rounded-2xl border-2 border-gray-100 p-4 sm:p-6 md:p-8 bg-white no-print overflow-x-auto w-full">
            <h3 className="text-[10px] font-black uppercase text-gray-400 tracking-[0.3em] mb-4 md:mb-6">Coating Performance Guide</h3>
            <table className="w-full min-w-[500px] text-xs md:text-sm text-center border-collapse">
              <thead>
                <tr className="text-[9px] md:text-[10px] font-black uppercase text-gray-400 border-b-2 border-gray-50">
                  <th className="p-2 md:p-4 text-left tracking-widest">Premium Feature</th>
                  <th className="p-2 md:p-4">Standard</th>
                  <th className="p-2 md:p-4">MAR Coating</th>
                  <th className="p-2 md:p-4">Blue Filter</th>
                </tr>
              </thead>
              <tbody className="font-bold text-slate-700">
                <tr className="border-b-2 border-gray-50">
                  <td className="p-3 md:p-5 text-left text-[10px] md:text-xs uppercase tracking-tight">Scratch Protection</td>
                  <td className="p-3 md:p-5 text-[#3f9185] text-lg md:text-xl">✓</td>
                  <td className="p-3 md:p-5 text-[#3f9185] text-lg md:text-xl">✓</td>
                  <td className="p-3 md:p-5 text-[#3f9185] text-lg md:text-xl">✓</td>
                </tr>
                <tr className="border-b-2 border-gray-50">
                  <td className="p-3 md:p-5 text-left text-[10px] md:text-xs uppercase tracking-tight">Anti-Reflection</td>
                  <td className="p-3 md:p-5 text-gray-200">-</td>
                  <td className="p-3 md:p-5 text-[#3f9185] text-lg md:text-xl">✓</td>
                  <td className="p-3 md:p-5 text-[#3f9185] text-lg md:text-xl">✓</td>
                </tr>
                <tr className="border-b-2 border-gray-50">
                  <td className="p-3 md:p-5 text-left text-[10px] md:text-xs uppercase tracking-tight">Easy-Clean Tech</td>
                  <td className="p-3 md:p-5 text-gray-200">-</td>
                  <td className="p-3 md:p-5 text-[#3f9185] text-lg md:text-xl">✓</td>
                  <td className="p-3 md:p-5 text-[#3f9185] text-lg md:text-xl">✓</td>
                </tr>
                <tr>
                  <td className="p-3 md:p-5 text-left text-[10px] md:text-xs uppercase tracking-tight">Digital Screen Shield</td>
                  <td className="p-3 md:p-5 text-gray-200">-</td>
                  <td className="p-3 md:p-5 text-gray-200">-</td>
                  <td className="p-3 md:p-5 text-[#3f9185] text-lg md:text-xl">✓</td>
                </tr>
        ...
        <div className="bg-slate-900 text-white p-6 sm:p-8 md:p-10">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 md:mb-8 border-b border-slate-800 pb-4 md:pb-6 gap-4">
            <div>
              <h2 className="text-2xl md:text-3xl font-black uppercase tracking-tighter italic">Total Quote</h2>
              <p className="text-slate-500 font-black uppercase text-[9px] md:text-[10px] tracking-[0.2em] mt-1 md:mt-2">Patient: {customerName || 'Pending Selection'}</p>
            </div>
            <div className="flex flex-col sm:items-end gap-3 no-print w-full sm:w-auto">
              <span className="text-[9px] md:text-[10px] font-black px-4 py-1.5 bg-[#3f9185]/20 text-[#3f9185] border border-[#3f9185]/30 rounded-full uppercase tracking-widest text-center sm:text-right">
                {isOwnFrame ? 'Reglaze Service' : 'Complete Pair'}
              </span>
              <button 
                onClick={handleSaveQuote}
                className="flex items-center justify-center gap-2 px-5 py-2.5 bg-[#3f9185] hover:bg-[#2c6b62] text-white text-[10px] font-black uppercase tracking-widest rounded-lg shadow-lg transition-all transform hover:scale-105 w-full sm:w-auto"
              >
          ...
          <div className="mt-6 sm:mt-10 pt-6 sm:pt-8 border-t border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
            <span className="text-xl md:text-2xl font-black uppercase tracking-tighter">Grand Total</span>
            <span className="text-4xl md:text-5xl font-black text-[#3f9185] drop-shadow-[0_0_15px_rgba(63,145,133,0.3)]">£{totalCost.toFixed(2)}</span>
          </div>
        </div>
      </div>
    </div>
